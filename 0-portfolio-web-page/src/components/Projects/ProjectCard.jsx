...
const ProjectCard = ({ imageClass, title, description, techStack, livePreviewLink, codeLink }) => {
    ...
    const shortDescription = isLongText ? description.slice(0, maxLength) + '...' : description;
    ...
    return (
        <>
            <div className='project-card'>
                ...
                <div className="project-card-text">
                    <div className='project-card-text-text'>
                        <h4>{title}</h4>
                        <p>{!isLongText ? description : shortDescription}</p>
                        {isLongText && (
                            <button className='openModal' onClick={openModal}>Read more</button>
                        )}
                    </div>
            ...
            {isModalOpen && <Modal title={title} description={description} closeModal={closeModal} />}
        </>
    )
}
